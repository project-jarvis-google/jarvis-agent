...
def _execute_query(conn: Any, query: str) -> List[Dict[str, Any]]:
    ...
    cursor = conn.cursor()
    try:
        conn.autocommit = True # Ensure no lingering transactions
...
def profile_postgres_data(conn: Any, schema_name: str, schema_structure: Dict[str, Any], sample_size: int = 10000) -> Dict[str, Any]:
    ...
    for table_name, table_info in tables.items():
        ...
        for col_name in table_info.get("columns", {}):
            ...
            FROM (SELECT "{col_name}" FROM "{schema_name}"."{table_name}" LIMIT {sample_size}) as sampled;
            """
            try:
        ...
        for col_name in key_columns:
            if col_name in table_info.get("columns", {}):
                card_q = f'SELECT COUNT(DISTINCT "{col_name}") as unique_count FROM "{schema_name}"."{table_name}";'
                try:
    ...
    for fk in schema_structure.get("foreign_keys", []):
        ...
        if from_table and from_col and to_table and to_col:
            ...
            FROM (SELECT "{from_col}" FROM "{schema_name}"."{from_table}" WHERE "{from_col}" IS NOT NULL LIMIT {sample_size}) as s
            LEFT JOIN "{schema_name}"."{to_table}" t ON s."{from_col}" = t."{to_col}";
            """
            try:
        ...
        for col_name, col_info in table_info.get("columns", {}).items():
            ...
            if "char" in col_type or "text" in col_type:
                if "phone" in col_name.lower() or "zip" in col_name.lower() or "postal" in col_name.lower():
                    ...
                    FROM (SELECT "{col_name}" FROM "{schema_name}"."{table_name}" WHERE "{col_name}" IS NOT NULL LIMIT {sample_size}) as s
                    WHERE "{col_name}" ~ '[^0-9.-]';
                    """
                    try:
