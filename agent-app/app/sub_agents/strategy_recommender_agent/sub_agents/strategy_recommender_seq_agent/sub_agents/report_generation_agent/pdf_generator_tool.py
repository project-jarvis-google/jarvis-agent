...
def generate_and_save_pdf(json_report_string: str, tool_context: ToolContext) -> str:
    ...
    try:
        ...
        logger.info(f"JSON REPORT DATA loaded successfully.")
        ...
        styles.add(ParagraphStyle(name='SectionHeader', fontName='Helvetica-Bold', fontSize=12, spaceBefore=25, spaceAfter=0.25 * inch))
        ...
        styles.add(ParagraphStyle(name='MappingDetail', fontName='Helvetica', fontSize=9, leftIndent=0.5 * inch, spaceAfter=3))
        styles.add(ParagraphStyle(name='BulletPoint', fontName='Helvetica', fontSize=10, leftIndent=0.25 * inch, spaceAfter=12))
        styles.add(ParagraphStyle(name='SubBulletPoint', fontName='Helvetica', fontSize=10, leftIndent=0.75 * inch, spaceAfter=12))
        
        # --- NEW STYLES FOR TABLE ---
        styles.add(ParagraphStyle(name='TableCell', fontName='Helvetica', fontSize=9, leading=10)) 
        styles.add(ParagraphStyle(name='TableHeader', fontName='Helvetica-Bold', fontSize=9, leading=10))
        ...
        if pain_points or desired_outcomes:
            ...
            if pain_points:
                ...
                story.append(Paragraph(f"<b>{key_findings_header}</b>", styles['StrategyTitle'])) 
                
                # Ensure the contents of the pain point list are stripped too
                pain_point_items = [Paragraph(point.strip() if isinstance(point, str) else point, styles['BulletPoint']) for point in pain_points]
                story.append(ListFlowable(pain_point_items, bulletAnchor='middle', bulletkind='bullet', leftIndent=0.25 * inch, spaceAfter=10))

            # 2. Render Desired Outcomes
            if desired_outcomes:
                story.append(Paragraph("<b>Desired Business Outcomes:</b>", styles['StrategyTitle'])) # Using StrategyTitle for bold subheading
                # Ensure the contents of the outcome list are stripped too
                outcome_items = [Paragraph(outcome.strip() if isinstance(outcome, str) else outcome, styles['BulletPoint']) for outcome in desired_outcomes]
                story.append(ListFlowable(outcome_items, bulletAnchor='middle', bulletkind='bullet', leftIndent=0.25 * inch, spaceAfter=15))
        ...
        def render_recommendation(r, section_type):
            ...
            justification_header = JUSTIFICATION_HEADERS.get(cleaned_section_type, "<b>Justification:</b>")
            ...
            if strategy == 'REFACTOR' and isinstance(justification_content, list) and justification_content and isinstance(justification_content[0], dict) and 'category' in justification_content[0]:
                ...
                for item in justification_content:
                    row = [
                    Paragraph(item.get(field, 'N/A').strip() if isinstance(item.get(field, 'N/A'), str) else str(item.get(field, 'N/A')), styles['TableCell'])
                ...
                for item in justification_content:
                    ...
                    if steps and isinstance(steps, list):
                        story.append(Paragraph(f"<u>{category}</u>:", styles['StrategyTitle']))
                        story.append(Spacer(1, 0.1 * inch))
                        ...
                        for i, step in enumerate(steps):
                            ...
                            if "Relevant Industry Solutions & Customer Stories:" in step.replace('*', ''):
                                ref_start_index = i
                                break
                        
                        main_steps = steps[:ref_start_index] if ref_start_index != -1 else steps
                        ref_items = steps[ref_start_index + 1:] if ref_start_index != -1 else []
                        ...
                        if ref_items:
                            ...
                            story.append(Paragraph("<b>Relevant Industry Solutions & Customer Stories:</b>", styles['StrategyTitle']))
                            ...
                            for ref_item in ref_items:
                                ...
                                if "For more details, visit:" in clean_ref_item:
                                    ...
                                    ref_paragraphs.append(Paragraph(ref_item.strip(), styles['BulletPoint']))
                            
                            if ref_paragraphs:
                                story.append(ListFlowable(ref_paragraphs, bulletAnchor='middle', bulletkind='bullet', leftIndent=0.25 * inch, spaceAfter=15))
                            ...
                            if final_link:
                                ...
                                if url_match:
                                    ...
                                    story.append(Paragraph(hyperlink_markup, styles['Justification']))
                                else:
                                    # Fallback to the original text if URL extraction fails
                                    story.append(Paragraph(final_link, styles['Justification']))
                            
                            
                    # 3. Render Common Modernization Patterns (Moved outside the 'if steps' block)
                    if patterns and isinstance(patterns, list):
                        story.append(Spacer(1, 0.1 * inch))
                        story.append(Paragraph("<b>Common Modernization Patterns:</b>", styles['StrategyTitle']))
                        
                        pattern_paragraphs = []
                        for p in patterns:
                            if isinstance(p, dict) and 'pattern' in p and 'url' in p:
                                hyperlink_markup = f'<link href="{p["url"]}" color="blue"><u>{p["pattern"].strip()}</u></link>'
                                pattern_paragraphs.append(Paragraph(hyperlink_markup, styles['BulletPoint']))

                        story.append(ListFlowable(pattern_paragraphs, bulletAnchor='middle', bulletkind='bullet', leftIndent=0.25 * inch, spaceAfter=10))


                    story.append(Spacer(1, 0.25 * inch)) # Space after each category's deep dive
                # --- END: NEW IMPLEMENTATION STEPS SECTION (WITH LINK FIX) ---

        
            # 2. REPLATFORM Table Logic (Independent IF)
            if strategy == 'REPLATFORM' and isinstance(justification_content, list) and justification_content and isinstance(justification_content[0], dict) and 'migration_target' in justification_content[0]:
                ...
                field_names = ['migration_target', 'description', 'effort', 'key_benefits']
                ...
                for item in justification_content:
                    ...
                    for field in field_names:
                        ...
                        row.append(Paragraph(str(cell_content).strip(), styles['TableCell']))
                    table_data.append(row)
            ...
            if not table_rendered:
                if justification_content and isinstance(justification_content, list):
                    ...
                    justification_items = [Paragraph(point.strip() if isinstance(point, str) else point, styles['BulletPoint']) for point in justification_content]
                    story.append(ListFlowable(justification_items, bulletAnchor='middle', bulletkind='bullet', leftIndent=0.25 * inch, spaceAfter=5))
                else:
                    story.append(Paragraph(str(justification_content), styles['Justification']))
            ...
            if pros_list and isinstance(pros_list, list) and pros_list:
                ...
                pros_items = [Paragraph(pro.strip() if isinstance(pro, str) else pro, styles['BulletPoint']) for pro in pros_list]
                story.append(ListFlowable(pros_items, bulletAnchor='middle', bulletkind='bullet', leftIndent=0.25 * inch, spaceAfter=5))

            # Cons Section
            if cons_list and isinstance(cons_list, list) and cons_list:
                story.append(Paragraph("<b>Cons:</b>", styles['StrategyTitle']))
                cons_items = [Paragraph(con.strip() if isinstance(con, str) else con, styles['BulletPoint']) for con in cons_list]
                story.append(ListFlowable(cons_items, bulletAnchor='middle', bulletkind='bullet', leftIndent=0.25 * inch, spaceAfter=15))
        ...
        if tech_stack_data:
            ...
            for item in tech_stack_data:
                ...
                row = [
                    ...
                    Paragraph(_clean_strategy_string(item.get(field, 'N/A')), styles['TableCell'])
                    if field == 'strategy' 
                    # For all other fields, safely convert to string and strip basic whitespace ONLY
                    else Paragraph(str(item.get(field, 'N/A')).strip(), styles['TableCell']) 
                    for field in field_names
                ]
                table_data.append(row)
            ...
            table.setStyle(TableStyle([
                ...
                ('VALIGN', (0, 0), (-1, -1), 'TOP'), # Vertical align: TOP is better for multi-line cells
            ...
            story.append(Paragraph("No specific technology stack summary provided in the report.", styles['Justification']))
            story.append(Spacer(1, 0.2 * inch))
