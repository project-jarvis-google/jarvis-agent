...
def fetch_source_code_from_git_repo(git_repo_url: str, access_token: str, tool_context: ToolContext) -> bool:
    ...
    try:
        ...
        url_with_token = git_repo_url.replace("https://", f"https://{access_token}:x-oauth-basic@")
        ...
        logging.error("except 'secure_temp_repo_dir' in locals() => %s", ('secure_temp_repo_dir' in locals()))
        logging.error("except os.path.exists => %s", os.path.exists(secure_temp_repo_dir))
    ...
    logging.info("outside 'secure_temp_repo_dir' in locals() => %s", ('secure_temp_repo_dir' in locals()))
