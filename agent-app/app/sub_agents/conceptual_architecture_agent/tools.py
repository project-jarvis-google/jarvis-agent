...
def analyze_architecture_document(document_file_path: str) -> str:
    ...
    for pattern in knowledge_base:
        score = sum(
            1
            for driver in extracted_drivers
            if any(
                driver.lower() in kb_driver.lower()
                or kb_driver.lower() in driver.lower()
                for kb_driver in pattern["drivers"]
            )
        )

        if score > 0:
            scored_patterns.append(
                {"name": pattern["name"], "score": score, "details": pattern}
            )
