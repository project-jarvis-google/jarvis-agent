...
def map_capabilities_to_inventory(
    ...
    try:
        ...
        if not app_data:
            return [{"error": "CSV processed, but no valid 'App_Name' or 'App_Description' rows found."}]

        # --- 2. Get Embeddings ---
        app_names = [app['name'] for app in app_data]
        app_texts_for_embedding = [app['text'] for app in app_data]

        logger.info(f"Generating embeddings for {len(app_texts_for_embedding)} applications...")
        app_vectors = embedding_model.get_embeddings(app_texts_for_embedding)
        
        logger.info(f"Generating embeddings for {len(capabilities)} query capabilities...")
...
def generate_capability_report_csv(
    ...
    logger.info(f"Generating CSV report for {client_name} with {len(report_data)} items.")
    try:
