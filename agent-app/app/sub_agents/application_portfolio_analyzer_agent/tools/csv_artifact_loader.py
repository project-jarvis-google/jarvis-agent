...
async def load_application_csv_artifact(tool_context: ToolContext):
    ...
    if (
        ...
        and app_data.inline_data is not None
        and app_data.inline_data.data is not None
    ):
...
async def load_server_csv_artifact(tool_context: ToolContext):
    ...
    if (
        ...
        and server_data.inline_data is not None
        and server_data.inline_data.data is not None
    ):
        logger.info(f"server data => {(server_data.inline_data.data).decode('utf-8')}")
        tool_context.state["server_data"] = (server_data.inline_data.data).decode(
            "utf-8"
        )
